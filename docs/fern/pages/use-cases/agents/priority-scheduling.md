...
The strict tier is compared first. Within that tier, every request with a classifier-provided `due_at` is scheduled ahead of every request without one (an unset due time sorts last) before FCFS, LCFS, or Weighted Shortest Processing Time (WSPT) applies its configured policy key. Sustained deadline-bearing traffic can therefore starve non-deadline requests in the same class.
...
- Run at enough load for requests to wait in the router queue. Watch [`dynamo_frontend_router_queue_pending_requests`](../../reference/observability/metrics-catalog.mdx#router-queue-metrics) and confirm it is greater than zero during the measured window.
...
| Router queue never becomes non-empty. | Lower `--router-queue-threshold`, increase offered load, or check the SGLang `max_num_batched_tokens` caveat in [Router Configuration and Tuning](../../developer-guide/knowledge-base/modular-components/router/configuration-and-tuning.md#tuning-guidelines). |
