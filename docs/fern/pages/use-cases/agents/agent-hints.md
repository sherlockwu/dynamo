...
| `strict_priority` | Router pending-queue tier. Higher values always precede lower values. Within a tier, every request with a classifier-provided `due_at` is scheduled ahead of every request without one (unset sorts last) before the configured queue policy is applied, so sustained deadline-bearing traffic can starve non-deadline requests. |
...
Backend support is runtime-specific. For SGLang flags and behavior, see [SGLang for Agentic Workloads](../../developer-guide/knowledge-base/modular-components/backends/sglang/agents-on-sglang.md).
