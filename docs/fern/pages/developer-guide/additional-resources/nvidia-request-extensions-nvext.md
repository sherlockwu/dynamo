...
| `strict_priority` | `u32` | `None` | Router pending-queue tier. Higher values always precede lower values; within a tier, requests with a classifier-provided `due_at` are scheduled ahead of requests without one. Unset is equivalent to `0`. |
...
`(strict_priority, due_at, configured_policy_key)`: within a strict tier, every request with a classifier-provided due time is scheduled ahead of every request without one (an unset due time sorts last) before FCFS, LCFS, or WSPT orders requests. Sustained deadline-bearing traffic can therefore starve non-deadline requests in the same class.
