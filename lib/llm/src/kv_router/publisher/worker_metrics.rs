...
impl Drop for WorkerMetricsPublisher {
    fn drop(&mut self) {
        self.cancel.cancel();
    }
}
...
impl WorkerMetricsPublisher {
    ...
    pub fn publish(
        ...
        self.tx.send_modify(|current| {
            current.insert(dp_rank, metrics);
        });
        Ok(())
    }
    ...
    pub(super) fn start_metrics_publishing(&self, event_publisher: EventPublisher, worker_id: u64) {
        ...
        tokio::spawn(async move {
            tokio::select! {
                _ = cancel.cancelled() => {}
                _ = run_metrics_publishing(metrics_rx, event_publisher, worker_id) => {}
            }
        });
    }
}

async fn run_metrics_publishing(
    mut rx: tokio::sync::watch::Receiver<BTreeMap<DpRank, WorkerMetrics>>,
    ...
    loop {
        tokio::select! {
            ...
            _ = &mut publish_timer, if !pending_publish.is_empty() => {
                for (dp_rank, metrics) in std::mem::take(&mut pending_publish) {
                    publish_metrics(&event_publisher, worker_id, dp_rank, &metrics).await;
                }
            }
            _ = replay.tick(), if !current_metrics.is_empty() => {
                for (&dp_rank, metrics) in &current_metrics {
                    publish_metrics(&event_publisher, worker_id, dp_rank, metrics).await;
                }
            }
        }
...
async fn publish_metrics(
    ...
    if let Err(error) = event_publisher.publish(&active_load).await {
        tracing::debug!(
            ...
            dp_rank,
            %error,
            "failed to publish worker metrics"
        );
    }
}
...
mod tests {
    ...
    #[test]
    fn dropping_the_publisher_cancels_its_background_tasks() {
        let publisher = WorkerMetricsPublisher::new().unwrap();
